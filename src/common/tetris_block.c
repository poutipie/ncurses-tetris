...
void TetrisBlock_init_Square(TETRIS_BLOCK* self) {
    ...
    self->m_blk_size = 4;
    self->m_blk_x_coords = (int*)malloc(self->m_blk_size * sizeof(int));
    self->m_blk_y_coords = (int*)malloc(self->m_blk_size * sizeof(int));
    
    /* Top left corner */
    self->m_blk_x_coords[0] = 0;
    self->m_blk_y_coords[0] = 0;
    
    /* Top right corner */
    self->m_blk_x_coords[1] = 1;
    self->m_blk_y_coords[1] = 0;

    /* Bottom left corner */
    self->m_blk_x_coords[2] = 0;
    self->m_blk_y_coords[2] = 1;

    /* Bottom right corner */
    self->m_blk_x_coords[3] = 1;
    self->m_blk_y_coords[3] = 1;
...
void TetrisBlock_init_S(TETRIS_BLOCK* self) {
    ...
    /* Bottom left corner */
    self->m_blk_x_coords[2] = -1;
    self->m_blk_y_coords[2] = 1;

    /* Bottom right corner */
    self->m_blk_x_coords[3] = 0;
    self->m_blk_y_coords[3] = 1;
}

void TetrisBlock_destroy(TETRIS_BLOCK* self) {
    free(self->m_blk_x_coords);
    free(self->m_blk_y_coords);
    free(self);
}
...
void TetrisBlock_move(TETRIS_BLOCK* self, WINDOW* h_win, int x, int y) {
    ...
    for (int i = 0; i < self->m_blk_size; ++i) {
        movement_allowed &= x_within_bounds(
            h_win, self->m_blk_x_coords[i] + x + self->m_world_pos_x
        );
        movement_allowed &= y_within_bounds(
            h_win, self->m_blk_y_coords[i] + y + self->m_world_pos_y
        );
    }
...
void _TetrisBlock_rotate_operation(TETRIS_BLOCK* self, bool clockwise) {
    ...
    for(int i = 0; i < self->m_blk_size; ++i) {
        int temp_x = self->m_blk_x_coords[i];
        self->m_blk_x_coords[i] = self->m_blk_y_coords[i];
        self->m_blk_y_coords[i] = temp_x;

        if (clockwise) {
            self->m_blk_x_coords[i] *= -1;
        } 
        else {
            self->m_blk_y_coords[i] *= -1;
        }
    }
...
bool TetrisBlock_out_of_bounds(TETRIS_BLOCK* self, WINDOW* h_win) {
    ...
    for (int i = 0; i < self->m_blk_size; ++i) {
        within_bounds &= x_within_bounds(
            h_win, self->m_blk_x_coords[i] + self->m_world_pos_x
        );
        within_bounds &= y_within_bounds(
            h_win, self->m_blk_y_coords[i]+ self->m_world_pos_y
        );
    }
...
void TetrisBlock_draw(TETRIS_BLOCK* self, WINDOW* h_win) {
    ...
    for(int i = 0; i < self->m_blk_size; ++i) {
        wmove(
            h_win,
            self->m_world_pos_y + self->m_blk_y_coords[i],
            self->m_world_pos_x + self->m_blk_x_coords[i]);
        wprintw(h_win, "*");
    }
